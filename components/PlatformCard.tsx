  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-neutral-200/80 dark:border-slate-700">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-display text-primary-dark dark:text-white capitalize">{platform}</h3>
      </div>
            ...
            return (
              <div key={key}>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {value.map((tag) => (
                    <span key={tag} className="text-xs px-2 py-0.5 border border-neutral-200 dark:border-slate-700 text-primary rounded-full">{tag}</span>
                  ))}
                </div>
              </div>
            );
