...
export default function MeaningCard({ type, label, content }: MeaningCardProps) {
  return (
    <div className={`bg-white rounded-lg p-4 border-l-4 shadow-sm ${borderColors[type] || "border-l-neutral-300"}`}>
      <h3 className={`text-sm font-bold mb-2 ${labelColors[type] || "text-neutral-600"}`}>{label}</h3>
      <div className="space-y-1.5">
        {Object.entries(content).map(([key, value]) => {
          ...
          return <p key={key} className="text-sm text-neutral-700">{String(value)}</p>;
        })}
      </div>
    </div>
  );
